...
class CHMM(object):
    ...
    def update_T(self,alpha=0.2):
        """Update the transition matrix given the accumulated counts matrix."""
        new_T = self.C + self.pseudocount
        norm = new_T.sum(2, keepdims=True)
        norm[norm == 0] = 1
        new_T /= norm
        old_T = self.T if hasattr(self, "T") else new_T
        self.T = alpha * new_T + (1-alpha)*old_T
...
''' Changed version for forward pass only '''
...
def updateC(C, T, n_clones, mess_fwd, mess_bwd, x, a):
    ...
    for t in range(1, timesteps):
        ...
        ) # Compute FB message pass
        q /= q.sum() # Calculate the count value for time step t, updating the Xi value 
        C[aij, i_start:i_stop, j_start:j_stop] += q # Cumulated counts of the transition over time. Allows for the normalization step in update_T to be done over all the time steps simply
...
#         ) # Compute FB message pass
#         q /= q.sum() # Normalization
#         C[aij, i_start:i_stop, j_start:j_stop] += q # Recursive update of the parameters gamma
